...
- Produces:
  ...
  - `GET /api/counts` — returns the current `Json<DecisionCounts>` (read-only; used by the frontend on load and as a fallback).
...
**Interfaces:**
- Consumes the HTTP API from Tasks 8–10: `GET /api/photos`, `GET /api/photos/:id`, `GET /api/groups`, `GET /thumb/:id`, `GET /preview/:id`, `POST /api/decisions`, `GET /api/counts`, `POST /api/export`.
...
- **Known follow-ups (not blockers):** the duplicate-compare view above (the spec's side-by-side `1–9` keeper screen). All decision/keeper actions are reachable in v1 via the grid + `K` key; only the dedicated compare *layout* is deferred.
